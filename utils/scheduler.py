...
class FlowMatchScheduler:
    ...
    def _get_sigma(self, timestep, device):
        if timestep.ndim == 2:
            timestep = timestep.flatten(0, 1)
    ...
    def convert_x0_to_noise(
        ...
        """
        x0: predicted clean data [B, C, H, W]
        xt: input noisy data [B, C, H, W]
        timestep: [B]

        xt = (1-sigma)*x0 + sigma*noise
        noise = (xt - (1-sigma)*x0) / sigma
        """
        sigma = self._get_sigma(timestep, x0.device)
        noise = (xt - (1 - sigma) * x0) / (sigma + 1e-8)
        return noise
    ...
    def convert_noise_to_x0(
        ...
        x0 = (xt - sigma*noise) / (1-sigma)
        """
        sigma = self._get_sigma(timestep, noise.device)
        x0 = (xt - sigma * noise) / (1 - sigma + 1e-8)
        return x0
    ...
    def convert_velocity_to_x0(
        ...
        """
        velocity: predicted velocity v = noise - x0
        xt: input noisy data
        timestep: [B]
        ...
        x0 = xt - sigma * v
        """
        sigma = self._get_sigma(timestep, velocity.device)
